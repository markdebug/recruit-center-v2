...
type Notification struct {
	...
	UserID     uint                `gorm:"not null;index" json:"userId"` // 接收者ID
	UserType   UserType            `gorm:"not null" json:"userType"`     // 用户类型
...
type NotificationTemplate struct {
	ID         uint                `gorm:"primarykey" json:"id"`
	Code       string              `gorm:"size:50;uniqueIndex" json:"code"` // 模板代码
